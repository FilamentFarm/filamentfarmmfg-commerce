...
export async function generateMetadata({ params }: { params: { handle: string } }) {
  ...
  return {
    ...
    openGraph: url ? { type: 'website', url, title: product.title, description: product.description, images: [{ url, width, height, alt }] } : null
  };
}
        ...
        <div className="mx-auto max-w-screen-2xl px-4">
          <div className="flex flex-col rounded-lg border border-neutral-200 bg-white p-8 md:p-12 lg:flex-row lg:gap-8 dark:border-neutral-800 dark:bg-black">
            <div className="h-full w-full basis-full lg:basis-4/6">
              <ProductImages images={product.images as Image[]} />
            </div>
          ...
          <li key={product.handle} className="aspect-square w-full flex-none min-[475px]:w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/5">
            <Link className="relative h-full w-full" href={`/product/${product.handle}`} prefetch={true}>
              <GridTileImage
                ...
                borderColor={clientConfig.theme.productButtonColor}
                borderWidth={clientConfig.theme.productCardBorderWidth}
              />
            </Link>
          </li>
        ))}
